...
export class TreeViewEventService {
    constructor(public dataService: TreeViewDataService) {}
        ...
        return new Promise((resolve, reject) => {
            ...
            if (!!item.items) {
                item.items.forEach((itemChild) => {
                    ...
                    return this.checkAllChildItems(
                        itemChild,
                        treeViewName
                    ).then(resolve, reject);
                });
        ...
        return new Promise((resolve, reject) => {
            items.forEach((itemChild) => {
                if (itemChild.items && itemChild.items.length > 0) {
                    ...
                    if (
                        itemChild.items.filter((e) => !e.checked).length !== 0
                    ) {
    ...
    private addCheckedItemsToCheckItemList(treeView: any[], treeViewName): any {
        return new Promise((resolve, reject) => {
            treeView.forEach((value, index) => {
                if (value.checked) {
                    this.dataService
                        .getItemsCheckedTempByName(treeViewName)
                        .push(value);
                }
                if (value.items && value.items.length > 0) {
                    return this.addCheckedItemsToCheckItemList(
                        value.items,
                        treeViewName
                    );
                }
            ...
            this.checkParentItems(treeView).then((response: any) => {
                this.addCheckedItemsToCheckItemList(
                    ...
                    if (
                        ...
                        typeof this.dataService.getCallBackByName(treeView) ===
                            'function'
                    ) {
                        this.dataService.getCallBackByName(treeView)();
                    }
                    ...
                    if (!!persistedName) {
                        this.writeOnLocalStorageCheckItems(
                            persistedName,
                            treeViewName
                        );
                    }
    ...
    private createArrayOfIdOfItemsCheckedByTreeViewName(
        treeViewName: string
    ): number[] {
        ...
        return this.dataService
            .getItemsCheckedByName(treeViewName)
            .map((e) => e.id);
    }

    private writeOnLocalStorageCheckItems(
        ...
        window.localStorage.setItem(
            persistedName,
            JSON.stringify({
                items: this.createArrayOfIdOfItemsCheckedByTreeViewName(
                    treeViewName
                ),
            })
        );
